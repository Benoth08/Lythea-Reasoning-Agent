...
from lythea.agentic.orchestrator import AgentOrchestrator, _MAX_STEPS
from lythea.agentic.workers import InProcessWorker, WorkerPool
...
class _FakeSandbox:
    ...
    def run_pytest(self):
        from lythea.agentic.sandbox import SandboxResult
        self._runs += 1
...
def test_plan_drop_removes_meta_steps():
    from lythea.agentic.orchestrator import _PLAN_DROP
...
def test_clean_synthesis_strips_solicitations_and_empty_fences():
    from lythea.agentic.orchestrator import _clean_synthesis
    raw = (
...
def test_pytest_failure_digest_extracts_assertion():
    from lythea.agentic.orchestrator import _pytest_failure_digest
    sample = (
...
def test_ssrf_guard_blocks_internal_hosts():
    import pytest
    from lythea.agentic.orchestrator import _assert_public_url, _host_is_public
...
def test_extract_py_block_and_filename():
    from lythea.agentic.orchestrator import _extract_py_block, _filename_from_code
    txt = ("Voici le code :\n```python\n"
           "def fibonacci(n):\n    a, b = 0, 1\n    for _ in range(n):\n"
           "        a, b = b, a + b\n    return a\n```\nVoilà.")
...
def test_extract_py_block_unlabelled_fence():
    from lythea.agentic.orchestrator import _extract_py_block
    code = _extract_py_block("```\nprint('hi')\n```")
    assert code == "print('hi')"


def test_jail_relpath_blocks_cross_mission_and_escapes():
    from lythea.agentic.orchestrator import _jail_relpath
...
def test_py_syntax_error_gate():
    from lythea.agentic.orchestrator import _py_syntax_error
...
def test_unified_diff_shows_noop():
    from lythea.agentic.orchestrator import _unified_diff
...
def test_parse_test_counts():
    from lythea.agentic.orchestrator import _parse_test_counts
...
def test_gen_batch_uses_batch_then_falls_back():
    import asyncio
    from lythea.agentic.orchestrator import AgentOrchestrator
...
def test_strip_think_handles_all_cases():
    from lythea.agentic.orchestrator import _strip_think
    # bloc fermé normal
...
def test_capture_think_includes_unclosed_tail():
    from lythea.agentic.orchestrator import _capture_think
...
def test_focus_failing_case_isolates_one_test():
    from lythea.agentic.orchestrator import _focus_failing_case
