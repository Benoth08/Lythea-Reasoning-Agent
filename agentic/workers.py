...
* :class:`InProcessWorker` wraps Lythéa's in-process ModelWrapper — the
  steered "Taëlys voice" core. It is the only worker that can carry the
  ``soft_memory`` prefix / activation-level features, so any step that
  must *be* Taëlys is routed here (``needs_prefix``).
...
from lythea.genlock import GENERATION_LOCK as _INPROCESS_LOCK
...
class InProcessWorker:
    """Wraps the shared, steered ModelWrapper (the Taëlys core)."""
