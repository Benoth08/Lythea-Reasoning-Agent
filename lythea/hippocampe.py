...
from lythea.cognition.consolidation import ConsolidationPhase
from lythea.cognition.encoding import EncodingPhase, EncodingResult  # noqa: F401
from lythea.cognition.generation import (
    QUESTION_STARTS,
    ReasoningGenerator,
    mask_open_tags as _mask_open_tags_fn,
    strip_reasoning,
)
from lythea.cognition.retrieval import RetrievalPhase
from lythea.cognition.storage import StoragePhase
from lythea.cognition.surprise import SurprisePhase
...
from lythea.git_sync import GitSync
from lythea.memory.kg import EntityExtractor, KnowledgeGraphStore
from lythea.memory.mhn import ModernHopfieldNetwork
from lythea.memory.retrieval import HybridRetriever
from lythea.memory.salience import SalienceFilter
from lythea.memory.sdm import SparseDistributedMemory
from lythea.model import HFModelWrapper, ImageCaptioner
from lythea.temporal import TemporalContext
from lythea.web import WebAgent, WebTriggerPolicy

log = logging.getLogger("lythea.hippocampe")


# Re-export from cognition.generation so external code that imports
# ``from lythea.hippocampe import strip_reasoning`` keeps working.
...
class Hippocampe:
    ...
    def __init__(
        ...
        from lythea.memory.visual_working_memory import (
        ...
        from lythea.config import DEFAULT_SAMPLING, SamplingProfile
        ...
        from lythea.cognition.deep_reasoning import DeepReasoningChain
        ...
        try:
            from lythea.config import PROCEDURAL_DIR
            from lythea.memory.procedural import ProceduralStore
        ...
        from lythea.microsleep import MicrosleepConfig, MicrosleepManager
        ...
        if getattr(s, "enable_cognitive_state", False):
            try:
                ...
                from lythea.memory.cognitive_state import (
                    CognitiveState,
        ...
        if getattr(s, "enable_inhibition", False):
            try:
                from lythea.cognition.inhibition import (
                    InhibitionConfig,
                    InhibitionFilter,
                    parse_whitelist,
                )
        ...
        if getattr(s, "enable_planning", False):
            try:
                from pathlib import Path
                from lythea.cognition.planning import (
                    GoalStack,
                    PlanGenerator,
                    PlanGeneratorConfig,
                    PlanningConfig,
        ...
        if getattr(s, "enable_predictive_coding", False):
            try:
                from lythea.cognition.predictive_coding import (
        ...
        if getattr(s, "enable_timeline", False):
            try:
                from lythea.cognition.timeline import (
                    TimelineConfig,
                    TimelineExtractor,
                )
        ...
        if getattr(s, "enable_metacognition", False):
            try:
                ...
                from lythea.cognition.metacognition import (
                    MetacognitionConfig,
                    MetacognitivePhase,
                )
    ...
    def _build_cascade_if_enabled(self, s: Any) -> Any:
        ...
        from lythea.cognition.cascade import CascadeGenerator
        from lythea.external.gemini_client import (
            GeminiClient,
            GeminiClientError,
            mask_api_key,
        )
    ...
    def cascade_status(self) -> dict[str, Any]:
        ...
        """
        from lythea.external.gemini_client import mask_api_key
        from lythea.settings import get_settings
        s = get_settings()
    ...
    def _phase_a_learn(
        ...
        if self._timeline is not None:
            try:
                from lythea.cognition.timeline import render_block as _tl_render
    ...
    def _emit_workspace_offers(
        ...
        """
        try:
            from lythea.cognition.mcp_integration import (
        ...
        try:
            from pathlib import Path
            from lythea.settings import get_settings
            from lythea.server.workspace import WorkspaceManager
            s = get_settings()
            sandbox_str = (
                getattr(s, "mcp_sandbox_dir", "")
                or str(Path.home() / ".lythea" / "sandbox")
    ...
    def _route_tool_v5_1(
        ...
        # Niveau 1 : sémantique
        try:
            from lythea.cognition.semantic_router import get_router
            router = get_router()
        ...
        try:
            from lythea.cognition.tool_dispatcher import dispatch_via_llm
    ...
    def _phase_c_assemble(
        ...
        if hasattr(self, "procedural_store") and self.procedural_store is not None:
            try:
                from lythea.memory.procedural import render_playbook
    ...
    def _generate_reasoning(
        ...
        if len(words) <= 6:
            from lythea.cognition.deep_reasoning import _ANALYTICAL_MARKERS
    ...
    def process_message(
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    if _s.web_classifier_enabled:
                        from lythea.cognition.web_classifier import (
                            looks_like_question,
                        )
                        if looks_like_question(user_intent):
                            ...
                            if self.mcp_manager is not None and self.mcp_loop is not None:
                                try:
                                    from lythea.cognition.mcp_integration import _FILENAME_RE
            ...
            try:
                from lythea.cognition.encoding import (
            ...
            if tool_chosen == "python" and self.model.is_loaded:
                ...
                try:
                    ...
                    if py_code:
                        from lythea.tools.python_executor import (
            ...
            try:
                from lythea.cognition.vision_active import (
            ...
            try:
                from lythea.cognition.deliberation import is_trivial_message
                _last_user = next(
            ...
            try:
                from lythea.cognition.reflection import (
                    reflect_on_response,
            ...
            try:
                from lythea.cognition.vision_active import detect_perceptual_uncertainty
