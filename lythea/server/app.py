"""FastAPI application with lifespan management and static file serving."""
...
from lythea.config import CACHE_ROOT, CHROMA_DIR, DATA_DIR, MHN_DIR, SDM_DIR
from lythea.env import PLATFORM
from lythea.git_sync import GitSync
from lythea.hippocampe import Hippocampe
...
from lythea.model import HFModelWrapper
from lythea.server.routes import router
from lythea.sessions import SessionManager
from lythea.web import WebTriggerPolicy

log = logging.getLogger("lythea.server.app")
...
class LytheaApp:
    ...
    def _init_soft_memory(self) -> None:
        ...
        """
        from lythea.settings import get_settings
        from lythea.soft_memory import SoftMemoryConfig, SoftMemoryTrainer, SoftPrefix
        s = get_settings()
...
async def lifespan(app: FastAPI):
    ...
    import os
    from lythea.boot import BootState, BootRunner
    ...
    try:
        from lythea.telegram_bot import start_if_configured
...
def create_app() -> FastAPI:
    ...
    from lythea.server.auth import AuthMiddleware, auth_banner
    from lythea.server.rate_limit import (
    ...
    app = FastAPI(title="Lythéa V4 — Taëlys", lifespan=lifespan)
