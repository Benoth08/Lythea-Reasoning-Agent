...
from lythea.config import CATALOG
from lythea.model import HFModelWrapper, vram_free_gb, vram_total_gb
from lythea.server.rate_limit import make_limit_decorator
from lythea.server.schemas import (
...
from lythea.sessions import Message
...
from lythea.settings import get_settings as _gs
_settings = _gs()
...
async def memory_health(request: Request) -> dict:
    ...
    app = _lythea(request)
    try:
        from lythea.memory.health import compute_health
...
async def cleanup_noise_entities(request: Request) -> dict:
    ...
    """
    import json
    from lythea.cognition.encoding import ENTITY_NOISE
...
async def list_models(request: Request) -> list[dict]:
    ...
    """
    from lythea.model import model_loadability_info
...
async def load_model(body: ModelLoadRequest, request: Request) -> StreamingResponse:
    ...
    async def stream():
        ...
        if error_ref:
            ...
            # take precedence.
            from lythea.config import DEFAULT_SAMPLING
...
async def chat(body: ChatRequest, request: Request) -> StreamingResponse:
    ...
    # prompt by Hippocampe so Taëlys reacts naturally to time gaps.
    last_message_ts = (
    ...
    if not examples:
        raise HTTPException(
            400, "Aucune donnée d'entraînement dans Chroma. "
                 "Discute un peu avec Taëlys d'abord pour accumuler de la mémoire.",
        )
...
async def clear_cache(request: Request) -> dict:
    ...
    from lythea.config import CACHE_ROOT
...
async def wipe_all_memory(request: Request) -> dict:
    ...
    try:
        from lythea.server.app import LytheaApp
        import chromadb
    ...
    try:
        ...
        if kg is not None:
            if hasattr(kg, "clear_in_place"):
                ...
                from lythea.memory.kg import KnowledgeGraphStore
...
async def list_captioners(request: Request) -> dict:
    from lythea.config import CAPTIONER_OPTIONS
    app = _lythea(request)
    captioner = app.hippocampe.captioner
    return {
...
async def set_cascade_key(request: Request) -> dict:
    ...
    from lythea.external.gemini_client import validate_api_key_format
...
async def upload_document(
    ...
    - ``mode=ingest`` — extrait + chunke + ingère dans ChromaDB +
      ...
      long-terme de Taëlys.
    """
    from lythea.server.document_ingest import (
...
async def workspace_upload(
    ...
    """
    from lythea.server.workspace import (
...
async def workspace_download(request: Request, path: str) -> StreamingResponse:
    ...
    """
    from lythea.server.workspace import WorkspacePathError
    manager = _workspace_manager(request)
    try:
...
async def workspace_ingest(request: Request) -> dict:
    ...
    from lythea.server.document_ingest import ingest_document_to_memory, is_supported
...
# ══════════════════════════════════════════════════════════════════════
# Taëlys emits one fenced block per file (marqueur « # file: chemin »).
...
async def codegen_extract(body: CodegenRequest, request: Request) -> dict:
    ...
    """
    from lythea.server.codegen import extract_code_files
...
async def codegen_zip(body: CodegenRequest, request: Request) -> StreamingResponse:
    ...
    from lythea.server.codegen import build_zip, extract_code_files
    files = extract_code_files(body.text)
    if not files:
        raise HTTPException(
            status_code=400,
...
async def codegen_commit(body: CodegenCommitRequest, request: Request) -> dict:
    ...
    """
    from lythea.server.codegen import extract_code_files
    from lythea.server.workspace import WorkspaceError
...
def _agent_orchestrator(request: Request):
    ...
    if ao is not None:
        return ao
    from lythea.agentic import AgentOrchestrator, WorkerPool
    from lythea.settings import get_settings
...
def _steering_engine(request: Request):
    ...
    if eng is not None:
        return eng
    from lythea.steering import SteeringEngine
