...
from lythea.agentic.skills import SkillLibrary
...
def _sandbox_mode() -> str:
    try:
        from lythea.agentic.sandbox import isolation
...
class AgentOrchestrator:
    ...
    def _make_sandbox(self, mission_dir):
        ...
        from lythea.agentic.sandbox import make_mission_sandbox
    ...
    def _seed_attachments(self, subdir, attachments):
        ...
        if not attachments or self.workspace is None:
            return [], ""
        from lythea.agentic.ingest import ingest_attachment
    ...
    async def _react(self, task: str, run_id: str, run, core, attachments=None):
        from lythea.agentic import tools as T
        ...
        from lythea.agentic.blackboard import MissionBlackboard
        ...
        from lythea.agentic.router import route as _route_task
        ...
        def op_tests():
            ...
            from lythea.agentic.sandbox import (  # noqa: PLC0415
        ...
        def op_websearch(query, max_results=5):
            ...
            try:
                from lythea.web_providers.factory import get_default_provider
        ...
        _tools_block = (
            "Tu es Taëlys, ingénieure agentique. Tu disposes de ces outils "
        ...
        from lythea.agentic.progress import ProgressTracker
        ...
        while it < max_iters:                     # extensible on real progress
            ...
            if not calls:
                ...
                if build_mode:
                    ...
                    if (final_ok and not _web_capped
                        ...
                        try:
                            from lythea.agentic.verifier import check_deliverable
            ...
            for call in _to_run:
                ...
                if call["name"] == "run_tests" and isinstance(result.get("result"), dict):
                    ...
                    if r.get("ok"):
                        ...
                        from lythea.agentic.blockage import (
            ...
            if pending_delib:
                ...
                if not thinking_mode:
                    try:
                        from lythea.cognition.deliberation import (
    ...
    async def run(self, task: str, *, run_id: str | None = None, subdir: str = "",
        ...
        try:
            ...
            # 1. PLAN (steered core — the plan is Taëlys reasoning).
            plan_prompt = (
